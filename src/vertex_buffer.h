...
void vertex_buffer_commit( struct vertex_buffer *buffer )
{
	...
	glBufferData( GL_ARRAY_BUFFER, buffer->vertex_count * sizeof( vertex ), buffer->vertices, GL_STATIC_DRAW );
...
struct vertex_buffer_handler vertex_buffer_push( struct vertex_buffer *buffer, vertex *vertices, uint32_t vertex_count, uint32_t *indices, uint32_t indice_count )
{
	...
	//Allocate new memory
	vertex   *new_vertices = (vertex*)malloc( sizeof( vertex ) * ( new_vertex_count ) );
	uint32_t *new_indices =  (uint32_t*)malloc( sizeof( uint32_t ) * ( new_indice_count ) );
	
	
	const uint32_t vertice_copy_size = ( sizeof( vertex ) * buffer->vertex_count );
	...
	vertex*   new_vertices_append_dest = (new_vertices + handler.vertex_offset);
	uint32_t* new_indices_append_dest =  (new_indices + handler.indice_offset);
	
	
	//Append data to the buffer. 
	memcpy( new_vertices_append_dest, vertices, ( sizeof( vertex )   * vertex_count) );
